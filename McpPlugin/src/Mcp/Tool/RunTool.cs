...
namespace com.IvanMurzak.McpPlugin
{
    ...
    public partial class RunTool : MethodWrapper, IRunTool
    {
        ...
        private Dictionary<string, object?>? ConvertNamedParameters(IReadOnlyDictionary<string, JsonElement>? namedParameters)
        {
            ...
            try
            {
                // Get expected parameter names from the method
                var methodParams = Method?.GetParameters();
                var paramNameLookup = BuildParameterNameLookup(methodParams);

                return namedParameters.ToDictionary(
                    keySelector: kvp => NormalizeParameterName(kvp.Key, paramNameLookup),
        ...
        /// <returns>A dictionary mapping lowercase names to actual parameter names.</returns>
        private static Dictionary<string, string>? BuildParameterNameLookup(ParameterInfo[]? methodParams)
        {
        ...
        private static string NormalizeParameterName(string incomingName, Dictionary<string, string>? paramNameLookup)
        {
