...
| `CurrentProjectPin` | **frozen at `initialize`** | Middleware-only. Correct by construction: the pin is a path segment of the client's configured URL, so every request of a session carries the same one. `RunSessionHandler` re-parses it into a local for the reaper (`StreamableHttpTransportLayer.cs:144`) and deliberately does not publish it. |
