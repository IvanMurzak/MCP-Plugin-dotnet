...
|---|---|---|---|---|---|
| **P1** | `StreamableHttpTransportLayer`: `PerSessionExecutionContext` `true` → **`false`** | red | **red** | 17/23 — *exactly* the two discriminating cells in **both** auth modes: `(init=False, list=True, expect=False)` and `(init=True, list=False, expect=True)`; the two non-discriminating diagonal cells stayed **green** | `Shouldly.ShouldAssertException : attributed.TryGetProperty("_meta", out var meta)` |
