...
| `plugin-timeout` | `MCP_PLUGIN_CLIENT_TIMEOUT` | Timeout for a SINGLE plugin invocation (ms). The bridge retries up to 10x with a 1s delay, so a connected-but-silent plugin can hold one call for roughly 10x this value. The four catalog endpoints (`tools/list`, `resources/list`, `resources/templates/list`, `prompts/list`) are separately capped at 15s and degrade to an empty catalog instead of failing the call. Note that cap is FIXED: raising this above 15000 does not raise it, so a plugin slower than 15s will list an empty catalog. | `10000` |
